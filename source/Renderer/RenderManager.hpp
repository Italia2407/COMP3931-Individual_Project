...
{
public:
    RenderManager(RTCDevice* device, Camera camera, bool smoothShading, u_int32_t multisamplingIterations, u_int16_t maxRayDepth, int photonNumPerLight);
...
public:
    ...
    void RenderScene(std::string outputFileName, u_int32_t imgWidth, u_int32_t imgHeight, PhotonMappingMode pmMode, int pmIterations);

private:
    //glm::vec3 TraceRay(glm::vec3 origin, glm::vec3 direction, float near, float far, u_int16_t& rayDepth);
    glm::vec3 CastRay(glm::vec3 origin, glm::uvec2 imageLocation, glm::vec3 direction, float near, float far, PhotonMappingMode pmMode, RTCIntersectContext& context, u_int16_t rayDepth);
    ...
    glm::vec3 CalculateReflectionColour(glm::vec3 hitPoint, glm::uvec2 imageLocation, glm::vec3 reflectionDirection, MaterialProperties surfaceProperties, PhotonMappingMode pmMode, RTCIntersectContext& context, u_int32_t rayDepth);
    glm::vec3 CalculateRefractionColour(glm::vec3 hitPoint, glm::uvec2 imageLocation, glm::vec3 surfaceNormal, glm::vec3 incidenceDirection, MaterialProperties surfaceProperties, PhotonMappingMode pmMode, RTCIntersectContext& context, u_int32_t rayDepth);
};